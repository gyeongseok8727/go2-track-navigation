...
# === 사용자 환경 설정 ===
MAP_YAML = "/home/mr/nav2_ws/my_map.yaml"
...
def main():
    try:
        ...
        # 6) Self Pose estimation
        # Waits for /Map to be published first
        time.sleep(2)  # 아주 긴 지연
        # wait_map_cmd = f"""{ROS_SETUP} && ros2 topic echo /map > /dev/null""" # (Jaewon)
        # print("[wait] waiting for /map to be published ...") # This is not printing properly (Jaewon)
        # ret = subprocess.call(["bash", "-lc", wait_map_cmd])
        # if ret != 0:
        #     print("[warn] /map wait exited abnormally")
        # else:
        #     print("[info] /map is available")
        # Map is now available
        end_of_jump = f"""{ROS_SETUP} && ros2 topic pub -1 /initialpose geometry_msgs/PoseWithCovarianceStamped "{{
                        header: {{
                        ...
                        pose: {{
                            pose: {{
                            position: {{x: -3.7144763759879127, y: -2.386646797201553, z: 0.0}},
                            orientation: {{x: 0.0, y: 0.0, z: 0.9660407137914812, w: 0.2583898978235133}}
                            }},
                            covariance: [
        ...
        print("[init] auto_localize:", end_of_jump)
        subprocess.call(["bash", "-lc", end_of_jump])
        ...
        time.sleep(1)
        # 4) Navigation 실행
        nav_cmd = f"""{ROS_SETUP} && ros2 launch nav2_bringup navigation_launch.py"""
        ...
        time.sleep(7)
        # 5) junbeom.py 실행
        junbeom_cmd = f"""{ROS_SETUP} && python3 example_simple_waypoints_junbeom_1.py"""
