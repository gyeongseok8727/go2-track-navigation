...
YAW_TARGET = target_yaw(0.9999751939800637, 0.007043537785371638)
...
def main(args=None):
    ...
    try:
        ...
        time.sleep(0.5)
        YAW_TARGET2 = target_yaw(-0.7490970258126652, 0.6624602976168603)
        n2 = Turn_Left(yaw_target = YAW_TARGET2, duration_s=1.2)
        executor.add_node(n2)
        run_stage(executor, n2)  # 안전 타임아웃
        
        n3 = WallFollower(vx = 2.0, yaw_target=YAW_TARGET2, duration_s=1.0, use_one_side_lidar = False, forward_target = 0.5)
        executor.add_node(n3)
        run_stage(executor, n3)

        time.sleep(0.5)
        YAW_TARGET3 = target_yaw(-0.02257797029249791, 0.9997450851379421)
        n4 = Turn_Left(yaw_target = YAW_TARGET3, duration_s=1.2)
