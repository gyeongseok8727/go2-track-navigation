...
## Workspace Layout
메인 ws는 nav2_ws이고, 여기서 ros2_ws, unitree_rl_lab의 코드를 불러와 실행하도록 한다. 해당 README에선 별 다른 표시가 없으면 ~/nav2_ws 내의 경로이다.  

nav2_ws에선 전체 오케스트레이션 코드와 NAV2의 실행을 위한 nav2_bringup를 src안에 두고 토픽 메시지의 타입 정의를 위해 unitree_api, _go, _hg를 둔다.  

nav2_bringup 내의 launch파일에서 부르는 NAV2의 localization, navigation 모듈은 바이너리로 받아서 /opt/ros/humble/share/ 밑에 nav2로 시작하는 패키지들을 여럿 깔아둔다.

즉, 다음을 해두면 된다. (https://docs.nav2.org/getting_started/index.html)  

    sudo apt install ros-$ROS_DISTRO-navigation2  
    sudo apt install ros-$ROS_DISTRO-nav2-bringup  

**unitree_rl_lab은 유니트리 깃헙의 코드를 받아서 이용한다.**  

저자세 주행을 위해 강화학습시킨 정책 실행을 위해 FSM을 정의하는 것과 학습, 배포, FSM과 통합하여 빌드하기 위한 구조 및 예제가 유니트리 깃헙에 있다.  

main.cpp에서 기존 조이스틱 조건을 udp 브릿지에서 받는 숫자신호를 통해 진행하도록 수정하여 이용했다.  
(https://github.com/unitreerobotics/unitree_rl_lab)  

ros2_ws 내에선 udp 브릿지 노드, YOLO를 이용한 신호등 및 장애물 회피 코드를 두었다.

## Setup & Dependencies

**우분투 22.04에서 ros2로 humble**을 깔아서 쓴다.

미리 설치가 필요한 것은 **unitree 공식 깃허브**에서 다음과 같다.  
...
**nav2**도 필요하다. 바이너리로 다운 받아서 쓰려면 아래 둘을 깐다.  
...
unitree_rl_lab 내의 log의 .pt 파일들과 학습과정을 보여주는 TensorFlow 이벤트 파일 (tfevents file)은 용량 상 실제 사용하는 것만 남겨두고 지워서 올린다.  

nav2_ws와 ros2_ws의 build, install, log, pycache 등은 지우고 업로드 한다.
...
--> 이동은 코스 1에서와 마찬가지로 하드코딩, /laserscan을 이용해 장애물 위치 인식 후 회피하며 돌파  
...
--> 2D map과 2D laserscan을 이용하는 amcl로는 계단을 주행할 수 없어 하드코딩함  

**코스 6: 경사로 돌파 구간**  

**코스 7: 포트폴 주행 구간**  
--> nav2로 이동하려 했으나 불규칙한 지형에서 로봇이 기울어지면 localization이 불안정하여 p제어 하드코딩으로 변경  

**코스 8: 신호등 인지 구간**  
--> YOLO로 신호등 인식하여 통과  

**코스 9: 전력질주 주행 구간**  
--> SportClient로 달리기 모드 켠 뒤 통과  


### Runtime Pipeline 최종 실행 코드: run_localization_hardcoding0.py

위의 미션코스별 전략에서 밝혔듯이, 여러 방법을 대회장에서 시도하기 위해 각 전략별 코드를 작성해두었고, 따라서 최종으로 사용한 코드 외의 파일도 여럿 업로드해두었다.

사용자가 가담하여 실행하는 코드는 run_localization_hardcoding0.py 하나로, 단계별로 다른 코드를 자동 실행하는 오케스트레이션 스크립트이다. 파이썬 subprocess를 이용하여 순차적 미션 수행을 위한 컨트롤러 파일을 여럿을 두고, 각 파일이 또 다른 파일을 부르며 트리처럼 순차적으로 실행한다.


**출발**

localization켜고 /initialpose 보내서 localization 시도하고 (시작점 ~ 계단 진입까지의 맵인 my_map_edit.pgm에서 localization)
map0_total.py 실행
    unitree sdk의 SportClient 이용
    WallFollower노드로 /laserscan, /lowstate 구독하여 vy, wz 각각 p제어하며 이동
...
obstacle_lidar.py 실행
    ...
    /lowstate의 yaw값 대신에 localization이 안될지라도 /amcl_pose의 yaw값을 이용하는 이유는 출발점부터 누적된 /lowstate의 적분 드리프트 때문이다.

    장애물 탐지 및 회피 로직은 다음과 같다.
        로봇이 벽 중앙, yaw 기준 적당히 맞췄다는 가정하에 전방 영역(30도~150도)의 laserscan으로 전방거리를 계산하는데, cos로 투영시킨 전방거리 중 장애물로 인식할 임계거리를 각도마다 다르게 둔다.
        30~60, 120~150도는 40cm, 60~120도는 80cm로 임계거리를 두고, 해당 거리 내에 들어온 점들을 장애물로 인식하여 거리 평균을 내서 장애물 까지 전방거리를 구한다.
        전방에 장애물이 없으면 0.5로 전진, 장애물 보이면 50cm 앞까지 다가가서 멈추도록 vx p제어한 다음,
        더 좁은 75~105도 영역에서 수직 투영한 거리 60cm안에 장애물이 안 보일 때 빠르게 통과한다.

**저자세 구간 진입**

udp_bridge_node.py 켠 상태로 lowlevel.py 실행하면 내부에서 go2_ctrl 바이너리 실행.
    udp_bridge_node.py
        ros2_ws/src/run_policy/run_policy에 있다.
        udp 통신은 ip 127.0.0.1의 9000 포트, 즉 로봇이 아니라 bridge를 실행 중인 pc 본인에게 보내면, go2_ctrl 바이너리에서 이를 듣는다.
        /cmd_vel, /mode_cmd 토픽을 구독,
        /cmd_vel은 nav2에서가 아니라, lowlevel.py에서 p제어하며 내보내는 Twist 메시지이다.
        /mode_cmd는 RL policy를 실행하기 위해 go2_ctrl 바이너리에 정의해둔 FSM에서 state를 전환하는 명령이다. 즉, 학습시킨 저자세 주행 강화학습 코드의 실행과 종료를 신호준다.

    lowlevel.py. go2_ctrl 바이너리
        /laserscan, /lowstate 구독,
        unitree sdk의 SportClient, MotionSwitcherClient 연결
        Set_rpy 노드는 /lowstate를 통해 yaw_target을 구한다.
            /lowstate를 통해 일정 시간 중 yaw값을 평균을 내어 drift로 누적된 lowstate의 오차와 상관없이 지금의 yaw값으로 yaw_target을 삼는다. 
        WallFollower 노드에서 RL policy를 실행하고 /scan, /lowstate를 받아서 /cmd_vel을 pub한다.
            sport client로 엎드리고, 
            motion switcher client로 sport client에서 벗어나고, 
            unitree_rl_lab/deploy/robots/go2/build/go2_ctrl을 실행한다.
            그 상태로
            /mode_cmd로 1, 2를 차례로 pub하여 go2_ctrl 내 FSM에서 RL policy를 실행하게한다.
            WallFollower 노드를 23초 실행하는데, 앞의 벽으로부터 60cm 부근에 p제어하여 멈추도록 한다. 
            lowstate의 콜백으로 yaw wz를, laserscan의 콜백으로 vy를 p제어하고 /cmd_vel을 pub한다.  
            시간이 지나면 /cmd_vel로 정지신호를 주고, 
            /mode_cmd로 3을 pub하여 RL policy에서 벗어나 엎드리도록 한다.
            그리고 go2_ctrl 바이너리를 self.ex.stop()로 종료하고, 
            /mode_cmd로 4를 pub하여 udp_bridge_node.py를 끈다. 
            그 후 msc로 이의 제어권을 박탈한다.

    go2_ctrl 바이너리
        udp로 읽은 값은 버퍼에 저장하고, 버퍼 구조체는 vx, vy, wz, mode_cmd를 담도록 정의한다.
        udp_receiver.h에서
            float형식으로 3개가 오면 vx, vy, wz로 인식, 아니면 정수로 읽어서 mode_cmd에 저장한다.
        FSM을 정의하여 버퍼에서 받은 mode_cmd 값에 따라 3개의 state를 비가역적으로 전환한다.
            1 Passive -> FixStand
            2 FixStand -> Velocity(RLBase, 엎드린 상태로 속도명령 따르도록 학습시킨 policy)
            3 Velocity -> SitDown
            4 udp_bridge_node.py에서 udp로 보내기 전에 스스로 코드 종료(ctrl + c)
...
go2_move.py 실행
    mission이 없을 땐 cmd_vel을 구독하여 SportClient로 로봇에 운동명령을 내린다.
...
이후 kill_all()로 localization, navigation, go2_move.py를 전부 끈다.
...
### 추가 코드 설명
최종 방식에 사용하진 않았지만 개발해둔 다른 코드 몇몇의 쓰임과 설명을 적어둔다.  
개발의 부산물로 남은 코드이므로 깔끔하지 않고, 따라서 여기부터의 README는 참고용으로만 적어둔다.


#### total_sequence.py
최종으로 사용한 run_localization_hardcoding0.py 대신에, 하드코딩 부분을 nav2를 이용하여 시도하기 위해 작성한 오케스트레이션 코드이다.  
마찬가지로 순서에 따라 다른 코드들을 불러 사용한다.

이를 이용한 전체 흐름은 다음과 같다.
    ...
    total_sequence.py (~/map5.yaml)
        ...
        **이후부터는 주석해두었고, 다음의 코스에서 필요한 코드를 붙여두지 않았다.**
            stair.py
                그냥 10초 올라가기
            ...
            avoid_go_oneshot_move.py
                이것도 실제 과정과 달리 끝에 두었다.


전체 흐름 말고 이 코드만을 설명하면 total_sequence.py를 실행하면  
...
기타로 make_pose(x, y, ori_z, ori_w) 함수는 geometry_msgs/msg/PoseStamped로 변환한 값을 리턴한다.


#### run_nav2_sequence_1.py
