...
REGISTER_OBSERVATION(velocity_commands)
{
    ...
    auto clip = [&](float v, const char* key){
        return std::clamp(
            v,
            ranges[key][0].as<float>(),
            ranges[key][1].as<float>());
    };

    // UDP 최신값 사용
    const float vx = cmd_buffer().vx.load();
    const float vy = cmd_buffer().vy.load();
    const float wz = cmd_buffer().wz.load();

    obs[0] = clip(vx, "lin_vel_x");
    obs[1] = clip(vy, "lin_vel_y");
    obs[2] = clip(wz, "ang_vel_z");
    return obs;
}
