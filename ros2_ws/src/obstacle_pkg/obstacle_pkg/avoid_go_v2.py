...
class ObstacleDecisionNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('cmd_topic', '/go2/sport_cmd')  # Int32: 3=GO, 2=STOP
    ...
    def _sig_handler(self, signum, frame): #for safe shutdown, use signum stuff
    ...
    def decision_worker(self):
        N = 5                 # sliding window
        K_SAFE = 3             # 최근 N개 중 안전 판정 >= K
        HOLD_SAFE_SPOT = 0.3   # 안전 지속 시간(초)
        DROP_OUT_MS = 400      # 진행 중 안전 상실 허용 시간(ms)
        MIN_IOU = 0.1          # 같은 장애물 일관성
