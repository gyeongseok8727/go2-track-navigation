            ...
            try:
                ...
                if chosen is not None and not self.started.is_set():
                    ...
                    if is_safe_spot(chosen["xc"], chosen["yc"]):
                        ...
                        if self.auto_exit_sec > 0.0 and self._exit_timer is None:
                            self._exit_timer = self.create_timer(self.auto_exit_sec, self._on_exit_timer)
    ...
    def video_loop(self):
        ...
        while not self.stop_ev.is_set():
            ...
            if code != 0:
                ...
                #induce spin to stop
                threading.Thread(target=lambda: (time.sleep(0.05), rclpy.shutdown()), daemon=True).start()
                break
