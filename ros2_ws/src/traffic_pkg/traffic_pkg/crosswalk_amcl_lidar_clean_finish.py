...
class CrossingNode(Node):
    ...
    def yaw_callback(self, msg):
        if not self.started.is_set() or self.done:
            return
    ...
    def _heartbeat(self):
        ...
        self.get_logger().info(
            f"q(raw={self.raw_q.qsize()}, sig={self.sig_q.qsize()}), done={self.done}, started={self.started.is_set()}"
        )
...
def main(args=None):
    ...
    try:
        ...
        run_stage(executor, node, timeout_s=120)  # 필요 시 제한시간 부여
    finally:
        try:
            executor.shutdown()
        except Exception:
