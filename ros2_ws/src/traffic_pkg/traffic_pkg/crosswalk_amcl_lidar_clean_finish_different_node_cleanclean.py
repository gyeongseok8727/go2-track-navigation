...
sc.SetTimeout(10.0)
sc.Init()
print("Initializing Unitree SportClient...")
...
class RunningNode(Node):
    ...
    def lidar_cb(self, msg: LaserScan):
        ...
        elif self.use_side_lidar and left_dist > 1.2 and right_dist > 1.2:
...
def main(args=None):
    ...
    node2 = RunningNode(yaw_target, use_one_side_lidar = True, use_side_lidar = False)
    executor.add_node(node2)
    # 기존 spin() 대신 — 두 번째 코드 스타일
    run_stage(executor, node2, timeout_s=5)  # 필요 시 제한시간 부여
    
    node3 = RunningNode(yaw_target, use_one_side_lidar = True, use_side_lidar = True)
