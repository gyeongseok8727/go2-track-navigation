...
import os
os.environ["GLOG_minloglevel"] = "2"   # hide WARNING and below from NNPACK.cpp
os.environ["TORCH_CPP_LOG_LEVEL"] = "ERROR"  # silence C++ WARN logs
...
class CrossingNode(Node):
    def __init__(self):
        ...
        self.declare_parameter("cmd_topic", "/go2/sport_cmd") # Int32 command topic (3=GO, 2=STOP)
    ...
    def decision_worker(self):
        ...
        K_GREEN = 3           # need >=K green in last N
        HOLD_GREEN_S = 0.3    # min dwell before go
        ...
        while not self.stop_ev.is_set():
            try:
                ...
                #print("sig_q empty")
                #gotta add logics here!!!!
                now = time.time()
                if last_green_ts and (now-last_green_ts) * 1000.0 > DROP_OUT_MS:
